...
def ottParser(m_title, m_director, response=None):
    ...
    if soup.select(tag) != []:
        ...
        if movie_soup.select_one(watch_tag) is not None:
            view_tag = "#root > div > div.css-1xm32e0 > section > div > div.css-10ofaaw > div > div > div > div:nth-child(1) > div.css-wpsvu8 > div.css-1nhig6u-RoundedCornerBlock-RoundedCornerBlock > div > section.w_exposed_cell.css-1impywp > div.css-usdi1z > div.css-9dnzub > div > div > div > ul > li > a"
            view = movie_soup.select(view_tag)
...
def theaterParser(response=None):
    ...
    theater_list = { "CGV" : False, "롯데시네마" : False, "메가박스" : False }
    for theater in theaters :
        t = theater.text.split(" ")[0]

        if t in theater_list.keys():
            if theater_list[t] == False:
                theater_list[t] = True
